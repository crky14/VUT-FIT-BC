...
**/
	...
	//vkladany uzol este neexistuje
	if(found == FALSE){
		//inicializacia prvku
		tBTNodePtr insert = malloc(sizeof(struct tBTNode));
		insert->Cont = Content;
		insert->LPtr = NULL;
		insert->RPtr = NULL;
		//uzol je prazdny
		if(position == NULL) 
			(*RootPtr) = insert;
		//vlozenie do laveho podstromu
		else if(position->Cont > Content)
			position->LPtr = insert;
		//vlozenie do praveho podstromu
		else
			position->RPtr = insert;
	}
}
...
**/
	//pushuje a vypisuje az kym nenajde najlavejsi
	while(ptr != NULL) {
		SPushP(Stack, ptr);
		BTWorkOut(ptr);
		ptr = ptr->LPtr;
...
**/
	...
	tBTNodePtr tmp;
	//kym nie je prazdny stack tak vypisuje podla preorder
	while(!SEmptyP(&stack)) {
		tmp = STopPopP(&stack);
...
** zásobníku. 
**/
	//pushuje kym nenajde najlavejsi
	while(ptr != NULL) {
		SPushP(Stack, ptr);
		ptr = ptr->LPtr;
	}		
...
**/
	...
	tBTNodePtr tmp;
	// vypisuje podla Inorder kym nie je prazdny zasobnik
	while(!SEmptyP(&stack)) {
		tmp = STopPopP(&stack);
		BTWorkOut(tmp);
...
**/
	//pushuje na oba zasobniky aj prvok aj ci uz bol navstiveny
	while(ptr != NULL) {
		SPushP(StackP, ptr);
		SPushB(StackB, TRUE);
		ptr = ptr->LPtr;
...
**/
	...
	tBTNodePtr tmp;
	bool bool_tmp;
	//vypis podla postorder kym je nieco na zasobniku
	while(!SEmptyP(&stackP)) {
		tmp = STopPopP(&stackP);
		bool_tmp = STopPopB(&stackB);
		if(bool_tmp == TRUE) {
			SPushP(&stackP, tmp);
			SPushB(&stackB, FALSE);
...
**/
	...
	tBTNodePtr tmp;
	//kym nie je zasobnik prazdny, ak nie je prvok na vrchu zasobniku NULL pushne oba jeho podstromya uvolni
	while(!SEmptyP(&stack)) {
		tmp = STopPopP(&stack);
		if(tmp == NULL) {
			continue;
