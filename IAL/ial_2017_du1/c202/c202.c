...
*/
	//kontrola ci pointer ukauje na zasobnik
	if(s == NULL) {
		stackError(SERR_INIT);
	}
	else {
		s->top = -1;	// inicializacia zasoniku na 0 prvkov
	}
}
...
*/
	return (s->top == -1) ? 42 : 0;	// terarny operator , vracia true ak je zasobnik prazdny (42)
}
...
*/	
	return (s->top == STACK_SIZE-1) ? 42 : 0; // vracia 42 ak je zasobnik plny
}
...
*/
	if(stackEmpty(s)) {
		stackError(SERR_TOP);	//ak je zasbnik prazdny vola funkciu stackError
	}
	else {
		*c = s->arr[s->top];	// prostrednictvom c vracia hodnotu z vrchu zasobniku
	}
}
...
*/
	if(!stackEmpty(s)) {
		s->top--;				// odstrani najvrchnejsi prvok zasobiku znizenim premenej top...
	}
}
...
*/
	if(!stackFull(s)) {
		s->top++;
		s->arr[s->top] = c;			//ak zasobnik nie je plny vklada na vrch hodnotu premennej c
	}
	else {
		stackError(SERR_PUSH);		// vola funciu stackError ak je zasobnik plny
	}
}
